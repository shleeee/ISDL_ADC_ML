...
def load_input_sym(input_file, bit, num_cursor):
	data_input = input_file
	bit=int(bit)
	with open(data_input,"r") as in_data:
		input_lines = in_data.readlines()
	...
	input_2D=np.transpose(input_2D)
	input_2D=input_2D/63
	return input_2D

def load_output_b(output_file, bit):
	...
	output_2D = np.zeros((bit,len(output)))
	output_1D = np.zeros((len(output)))
	for output_len in range(len(output_1D)) :
		output_2D[0:bit,output_len]=list(map(int,output[output_len]))	
		output_1D[output_len]=int(output[output_len],2)
	output_2D=np.transpose(output_2D)
	return output_2D, output_1D
