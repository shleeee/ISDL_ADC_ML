...
Y_train, y_train = io.load_output("./data/PAM4/27dB/data_in_5.txt",3) # 2x819124
tic = time.time()
...
print('Training Accuracy: ' + str(model.test_b(X_train, y_train, ML_EQ)*100))
...
X_test=io.load_input_sym("./data/PAM4/27dB_test/data_out_5.txt",6,5) # 30x819126
Y_test, y_test = io.load_output("./data/PAM4/27dB_test/data_in_5.txt",3) # 2x819124
...
print('Test Accuracy: ' + str(model.test_b(X_test, y_test, ML_EQ)*100))

#torch.save(ML_EQ.module.state_dict(), Save_PATH) # Saving Model
#io.write_weight(ML_EQ, "./result/ML_EQ_weight")
io.write_output(torch.round(ML_EQ(X_test)), "./result/ML_EQ_output")
