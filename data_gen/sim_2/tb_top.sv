...
integer seed;
	real ch_in, ch_out;
	assign ch_in = (prbs_out==2'b00) ? -3 + NOISE*2*$dist_normal(seed,0,1):
		       (prbs_out==2'b01) ? -1 + NOISE*2*$dist_normal(seed,0,1):
		       (prbs_out==2'b10) ? 1 + NOISE*2*$dist_normal(seed,0,1): 3 + NOISE*2*$dist_normal(seed,0,1);
